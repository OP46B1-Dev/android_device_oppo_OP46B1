...
// Read the single integer state from the fp_state node. Returns true when the
// touchpanel driver is signalling "finger on UDFPS icon".
static bool readFpState(int fd) {
    ...
    if (rc < 0) {
        ALOGE("failed to read fp_state: %d", errno);
        return false;
    }

    if (sscanf(buffer, "%d", &state) < 1) {
        ALOGE("failed to parse fp_state");
        return false;
    }
...
Sensor::Sensor(int32_t sensorHandle, ISensorsEventCallback* callback)
    ...
    constexpr int64_t kDefaultMaxDelayNs = 1000 * 1000 * 1000;
    mSensorInfo.maxDelay = kDefaultMaxDelayNs;
...
UdfpsSensor::UdfpsSensor(int32_t sensorHandle, ISensorsEventCallback* callback)
    ...
    // Fixed fingerprint icon center for this panel (config_udfps_sensor_props).
    mScreenX = 540;
    mScreenY = 2034;
    ...
    if (mPollFd < 0) {
        ALOGE("failed to open fp_state: %d", errno);
    }
...
bool SysfsPollingOneShotSensor::readFd(const int fd) {
    ...
    int rc = read(fd, &buffer, sizeof(buffer));
    if (rc < 0) {
        ALOGE("failed to read state: %d", errno);
        return false;
    }

    if (sscanf(buffer, "%d", &state) < 1) {
        ALOGE("failed to parse state");
        return false;
    }
